...
export default function StudentChatHub({ auth }) {
  ...
  const startPrivateChat = (student) => {
    // Redirect to direct chat with the teacher, which is the only DM allowed for students
    const teacherRoom = rooms.find(r => r.type === 'direct');
    if (teacherRoom) {
      setActiveRoom(teacherRoom);
    }
  };
  ...
  const filteredRooms = rooms.filter(room => 
    (room.name || "Teacher").toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="flex flex-col h-[75vh] min-h-[600px] bg-white rounded-[2.5rem] overflow-hidden border-[4px] border-[#1E1E1E] shadow-[8px_8px_0px_0px_#1E1E1E]">
            ...
            {filteredRooms.map((room) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-black text-sm truncate">{room.name || "Teacher Chat"}</h4>
        ...
        <section className="flex-1 flex flex-col bg-white">
          {activeRoom ? (
            <>
              <div className="p-4 bg-[#FFF8E7] border-b-[3px] border-[#1E1E1E] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black text-lg leading-none">{activeRoom.name || "Teacher Chat"}</h3>
                ...
                <div className="flex items-center gap-2">
                  {activeRoom?.type === 'group' && (
                    <button 
                      ...
                      className={`px-4 py-2 rounded-xl hover:bg-gray-100 ${brutalBorder} bg-white shadow-[2px_2px_0px_0px_#1E1E1E] font-black text-xs flex items-center gap-2 transition-all`}
                    >
                    ...
                    return (
                      <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'} group/msg`}>
                        <div className={`max-w-[75%] flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                          {!isMe && (
                             <span className="mb-1 ml-2 text-[10px] font-black text-gray-600 uppercase tracking-widest">{msg.sender_name || 'Teacher'}</span>
                          )}
                          <div className="flex items-center gap-2">
                            {isMe && (
                              <button 
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-gray-50">
              <div className="w-32 h-32 bg-white rounded-full border-[3px] border-[#1E1E1E] shadow-[4px_4px_0px_0px_#1E1E1E] flex items-center justify-center mb-8">
                 <Users size={64} className="text-[#1E1E1E]" />
              </div>
              <h2 className="text-4xl font-black mb-4">Student Hub</h2>
              <p className="font-bold text-gray-500 italic max-w-sm">Select a group or teacher to stay connected.</p>
            </div>
          )}
        </section>
